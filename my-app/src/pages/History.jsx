...
const TanksFishies = () => {
  ...
  useEffect(() => {
    ...
    // Ensure sessionData is an array before calling reduce
    if (Array.isArray(sessionData)) {
      const map = sessionData.reduce((map, session) => {
        map[session.type] = (map[session.type] || 0) + 1;
        return map;
      }, {});
      setTypeCountMap(map);
    } else {
      console.warn(`Invalid sessionData for ${activeComponent}:`, sessionData);
      setTypeCountMap({}); // Reset to an empty map if data is invalid
    }
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {state.daySession || state.weekSession || state.monthSession || state.yearSession
            ? <SeaTank creatureCounts={typeCountMap} />
